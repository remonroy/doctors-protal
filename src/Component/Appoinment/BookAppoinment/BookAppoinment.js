...
const BookAppoinment = ({date}) => {
    ...
    return (
        <div>
            <h1 style={{color:'#10CFE2'}} className="text-center">Avialable BookAppoinment on {date.toDateString()}</h1>
            <div className="row mt-5 mb-5">
                {
                    bookingData.map(book=><BookCard date={date} book={book} key={book.id}></BookCard>)
                }
            </div>
        </div>
    );
